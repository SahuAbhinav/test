			...
			function() {
				...
				$("#showreport").click(
						function() {
							...
							$.ajax({
								type : "POST",
								url : "attendance_report/html?monthName="+$('#salaryMonth').val().replace('&','%26')+"&yearName="+$('#salaryYear').val().replace('&','%26'),
									success : function(data) {
									$("#loadingImageHtmlReport").hide();
									$("#reportHtml").html(data.htmlAttendance);
								}
							});
							return false;
				...
				$("#downloadPdf").click(function() {
					var url = "attendance_report/pdf?monthName="+$('#salaryMonth').val().replace('&','%26')+"&yearName="+$('#salaryYear').val().replace('&','%26');
					try {
				...
				$("#downloadXls").click(function() {
					var url = "attendance_report/xls?monthName="+$('#salaryMonth').val().replace('&','%26')+"&yearName="+$('#salaryYear').val().replace('&','%26');
					try {
						var child = window.open(url);
						child.focus();
					} catch (e) {
				...
				$("#downloadCsv").click(function() {
					var url = "attendance_report/csv?monthName="+$('#salaryMonth').val().replace('&','%26')+"&yearName="+$('#salaryYear').val().replace('&','%26');
					try {
						var child = window.open(url);
						child.focus();
					} catch (e) {
...
<body>
 <div class="header">Attendance Calculation List Report</div> 
	<div class='accordion'>
		<h3>
