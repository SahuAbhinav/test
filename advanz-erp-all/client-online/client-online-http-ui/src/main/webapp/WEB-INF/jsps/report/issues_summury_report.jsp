...
<script type="text/javascript">
			...
			function() {
				...
				$("#showreport").click(
						function() {
							...
							$.ajax({
								type : "POST",
								url : "issues_summury_report/html?"+ "fromDate=" +  $("#fromDate").val()+ "&toDate=" +  $("#toDate").val()+ "&itemGroupName=" +  $("#itemGroupName").val().replace('&','%26')+"&itemGroupFlagName=" +  $("#itemGroupFlagName").val().replace('&','%26')+ "&name=" + $("#name").val().replace('&','%26')+ "&itemName=" +  $("#itemName").val().replace('&','%26')+"&itemcategory="+$("#itemCategoryName").val()+"&activeStatus="+$("#activeStatus").val()+"&headName="+$("#headName").val()+"&sectionName="+$("#sectionName").val()+"&rateType="+$("#rateType").val()+"&loanType="+$("#loanType").val(),
								success : function(data) {
				...
				$("#downloadPdf").click(function() {
					...
					var url = "issues_summury_report/pdf?"+"fromDate=" +  $("#fromDate").val()+ "&toDate=" +  $("#toDate").val()+ "&itemGroupName=" +  $("#itemGroupName").val().replace('&','%26')+"&itemGroupFlagName=" +  $("#itemGroupFlagName").val().replace('&','%26')+ "&name=" + $("#name").val().replace('&','%26')+ "&itemName=" +  $("#itemName").val().replace('&','%26')+"&itemcategory="+$("#itemCategoryName").val()+"&activeStatus="+$("#activeStatus").val()+"&headName="+$("#headName").val()+"&sectionName="+$("#sectionName").val()+"&rateType="+$("#rateType").val()+"&loanType="+$("#loanType").val();   
					try {
				...
				$("#downloadXls").click(function() {
					...
					var url = "issues_summury_report/xls?"+"fromDate=" +  $("#fromDate").val()+ "&toDate=" +  $("#toDate").val()+ "&itemGroupName=" +  $("#itemGroupName").val().replace('&','%26')+"&itemGroupFlagName=" +  $("#itemGroupFlagName").val().replace('&','%26')+ "&name=" + $("#name").val().replace('&','%26')+ "&itemName=" +  $("#itemName").val().replace('&','%26')+"&itemcategory="+$("#itemCategoryName").val()+"&activeStatus="+$("#activeStatus").val()+"&headName="+$("#headName").val()+"&sectionName="+$("#sectionName").val()+"&rateType="+$("#rateType").val()+"&loanType="+$("#loanType").val();
					try {
						var child = window.open(url);
						child.focus();
					} catch (e) {
		...
		$("#downloadCsv").click(function() {
			if ($("#fromDate").val() == '') {
				...
				if ($("#toDate").val() == '') {
					...
					var url = "issues_summury_report/csv?"+"fromDate=" +  $("#fromDate").val()+ "&toDate=" +  $("#toDate").val()+ "&itemGroupName=" +  $("#itemGroupName").val().replace('&','%26')+"&itemGroupFlagName=" +  $("#itemGroupFlagName").val().replace('&','%26')+ "&name=" + $("#name").val().replace('&','%26')+ "&itemName=" +  $("#itemName").val().replace('&','%26')+"&itemcategory="+$("#itemCategoryName").val()+"&activeStatus="+$("#activeStatus").val()+"&headName="+$("#headName").val()+"&sectionName="+$("#sectionName").val()+"&rateType="+$("#rateType").val()+"&loanType="+$("#loanType").val();
					try {
						var child = window.open(url);
						child.focus();
					} catch (e) {
	...
	<body>
	<div class="header">Issues Summary Report</div>
	<div class='accordion'>
		<h3>
		...
		<div class="headerbg">
			...
			<td width="100"><form:input path="issueReportFrom.fromDate"  id="fromDate" style="width:50pxl;"
							 size="16" readonly="true"/>
							</td>
			<td width="35" nowrap="nowrap">To Date:<span style="color: red;">*</span></td>
			<td width="100"><form:input path="issueReportFrom.fromDate"  id="toDate"
							 size="16" readonly="true"/>
							</td>
		...
		<form:select  path="issueReportFrom.itemCategory" id="itemCategoryName" style="width:120px;" >
			...
			<td width="135"><form:select path="issueReportFrom.rateType" id="rateType">
