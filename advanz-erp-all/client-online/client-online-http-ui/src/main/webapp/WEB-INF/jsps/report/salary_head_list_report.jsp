			...
			function() {
				...
				$("#showreport").click(
						function() {
							...
							$.ajax({
								type : "POST",
								url : "salary_head_list_report/html?HeadTypePrompt="+$('#HeadTypePrompt').val(),
				...
				$("#downloadPdf").click(function() {
					var url = "salary_head_list_report/pdf?HeadTypePrompt="+$('#HeadTypePrompt').val();
					try {
				...
				$("#downloadXls").click(function() {
					var url = "salary_head_list_report/xls?HeadTypePrompt="+$('#HeadTypePrompt').val();
					try {
						var child = window.open(url);
						child.focus();
					} catch (e) {
				...
				$("#downloadCsv").click(function() {
					var url = "salary_head_list_report/csv?HeadTypePrompt="+$('#HeadTypePrompt').val();
					try {
						var child = window.open(url);
						child.focus();
					} catch (e) {
...
<body>
<div class="header">Salary Head List Report</div>
	<div class='accordion'>
		<h3>
		...
		<!-- 	<tr>					
			...
			<td>Head Type:</td>
			<td><form:select path="salaryheadReportFrom.salaryheadDTO.headType" id="HeadTypePrompt" style="width: 250px; height: 23px;" >
				<form:option value="All" selected="selected">All</form:option>
				<form:options items="${salaryheadList}" itemLabel="headType" itemValue="headType"  />
			</form:select></td></tr>			
	</table>
