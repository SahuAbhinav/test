			...
			function() {
				...
				$("#showreport").click(
						function() {
							...
							$.ajax({
								type : "POST",
								url : "salary_slip/html?monthName="+$('#salaryMonth').val().replace('&','%26')+"&yearName="+$('#salaryYear').val().replace('&','%26'),
				...
				$("#downloadPdf").click(function() {
					var url = "salary_slip/pdf?monthName="+$('#salaryMonth').val().replace('&','%26')+"&yearName="+$('#salaryYear').val().replace('&','%26')+"&empId="+$('#employeeId').val().replace('&','%26');
					try {
				...
				$("#downloadXls").click(function() {
					var url = "salary_slip/xls?monthName="+$('#salaryMonth').val().replace('&','%26')+"&yearName="+$('#salaryYear').val().replace('&','%26');
					try {
						var child = window.open(url);
						child.focus();
					} catch (e) {
				...
				$("#downloadCsv").click(function() {
					var url = "salary_slip/csv?monthName="+$('#salaryMonth').val().replace('&','%26')+"&yearName="+$('#salaryYear').val().replace('&','%26');
					try {
						var child = window.open(url);
						child.focus();
					} catch (e) {
...
<body>
 <div class="header">Salary Slip  Report</div> 
	<div class='accordion'>
		<h3>
		...
		<tr>
			...
			<td>Employee:</td>
			<td>
				<form:select path="salaryMasterForm.salaryMasterDTO.employeeDTO.employeeId" id="employeeId"  style="width: 90px; height: 23px;" >
					<form:option value="All" selected="selected">All</form:option>
					<form:options items="${salaryEmployeeList}" itemLabel="employeeName" itemValue="employeeId"/>
				</form:select>
