...
//		alert($('#itemGroupName option:selected').text()); 
		...
		$("#showreport").click(
			...
			$.ajax({
				...
				type : "POST",
				url : "pending_sales_order_report/html?"+ "fromDate=" +  $("#fromDate").val()+ "&toDate=" +  $("#toDate").val()+ "&itemName=" +  $("#itemName").val()+ "&partyName=" + $("#partyName").val()+"&pendingStatus="+$("#pendingStatus").val()+"&itemcategory="+$("#itemcategory").val()+"&activeStatus="+$("#activeStatus").val()+"&orderType="+$("#orderType").val(),
			    success: function(data) {
				   ...
				   return false; 
					...
					$("#downloadPdf").click( function() {
						...
						var url = "pending_sales_order_report/pdf?"+ "fromDate=" +  $("#fromDate").val()+ "&toDate=" +  $("#toDate").val()+ "&itemName=" +  $("#itemName").val()+ "&partyName=" + $("#partyName").val()+"&pendingStatus="+$("#pendingStatus").val()+"&itemcategory="+$("#itemcategory").val()+"&activeStatus="+$("#activeStatus").val()+"&orderType="+$("#orderType").val();
					...
					$("#downloadXls").click( function() {
						...
						var url = "pending_sales_order_report/xls?"+ "fromDate=" +  $("#fromDate").val()+ "&toDate=" +  $("#toDate").val()+ "&itemName=" +  $("#itemName").val()+ "&partyName=" + $("#partyName").val()+"&pendingStatus="+$("#pendingStatus").val()+"&itemcategory="+$("#itemcategory").val()+"&activeStatus="+$("#activeStatus").val()+"&orderType="+$("#orderType").val();
					 		try { var child = window.open(url); child.focus(); } catch (e) { }
					 });	
					...
					$("#downloadCsv").click( function() {
						...
						var url = "pending_sales_order_report/csv?"+ "fromDate=" +  $("#fromDate").val()+ "&toDate=" +  $("#toDate").val()+ "&itemName=" +  $("#itemName").val()+ "&partyName=" + $("#partyName").val()+"&pendingStatus="+$("#pendingStatus").val()+"&itemcategory="+$("#itemcategory").val()+"&activeStatus="+$("#activeStatus").val()+"&orderType="+$("#orderType").val();
					 		try {
...
<body>
<div class="header">Pending Sales Order Report</div>
<div class='accordion'>
  <h3><a href='#'>Show Control</a></h3>
 
<div class="headerbg">
	
	<form:form commandName="salesReportForm" id="formId"
		...
		<table style="width:670px; float: left;">
			...
			<td width="85">Order Type</td>
			<td width="145"><select name="orderType" id="orderType">
				<option value="desire_delivery_date" >Desire Delivery Date</option>
				<option value="planned_deliver_date" >Planned Peliver Date</option>
			</select></td>
