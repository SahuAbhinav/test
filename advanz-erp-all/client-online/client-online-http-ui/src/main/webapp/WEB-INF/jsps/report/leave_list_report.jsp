...
$(document).ready(function() {
	var foption = $('select option:first');
	var soptions = $.makeArray($('select option:not(:first)')).sort(function(a, b) {
	    return a.text == b.text ? 0 : a.text < b.text ? -1 : 1
	});
	$('select').html(soptions).prepend(foption);
			...
			function() {
				...
				$("#showreport").click(
						function() {
							...
							$.ajax({
								type : "POST",
								url : "leave_list_report/html?LeaveTypePrompt="+$('#LeaveTypePrompt').val().replace('&','%26'),
				...
				$("#downloadPdf").click(function() {
					var url = "leave_list_report/pdf?LeaveTypePrompt="+$('#LeaveTypePrompt').val().replace('&','%26');
					try {
				...
				$("#downloadXls").click(function() {
					var url = "leave_list_report/xls?LeaveTypePrompt="+$('#LeaveTypePrompt').val().replace('&','%26');
					try {
						var child = window.open(url);
						child.focus();
					} catch (e) {
				...
				$("#downloadCsv").click(function() {
					var url = "leave_list_report/csv?LeaveTypePrompt="+$('#LeaveTypePrompt').val().replace('&','%26');
					try {
						var child = window.open(url);
						child.focus();
					} catch (e) {
...
<body>
<div class="header">Leave List Report</div>
	<div class='accordion'>
		<h3>
...
<table style="width: 550px; float: left;">
		<tr>
			<td>Leave Type:</td>
			<td><form:select path="leavetypeReportFrom.leavetypeDTO.leaveType" id="LeaveTypePrompt" style="width: 250px; height: 23px;" >
				<form:option value="All" selected="selected">All</form:option>
				<form:options items="${leaveList}" itemLabel="leaveType" itemValue="leaveType"  />
			</form:select></td></tr>			
</table>
