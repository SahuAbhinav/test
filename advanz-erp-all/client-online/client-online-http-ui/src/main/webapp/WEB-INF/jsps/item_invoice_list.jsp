...
<script type="text/javascript">
	...
	$(document).ready(function() {
		$(function() {
			$('.sr').click(function() {
				...
				$.get('get_item_detail_by_id', { id: $(this).attr('id')}, function(data) {					 					
					 					...
					 					$('.okbtn').click(function() {
					 						 ...
					 						 if(data.batchFlag == 1){
							 					    window.location='submitInvoiceItemBatchList?operation=show&itemId='+data.itemId;
							 					}
	...
	<%//show_invoice %>
	<form:form name="input" action="submitItemList" class="formdiv" method="GET" modelAttribute="itemForm">
    <div class="header">Invoice Item List</div> 
...
<form:hidden path="itemDTO.excisePerc" id="ExcisePerc"/>
    </td>
          <td width="35">
          ...
          <td width="67"><input class="searchbtn" type="submit" name="operation"  value="" onclick="this.value='Search',disableSalerOrderField();"/></td>
          <td width="42"><input class="okbtn" type="submit" onclick="disableOkButton()" name="operation" value="OK" />
          <input type="hidden" name="operation" value="OK" class="SalesOrderNoId" />
          </td>
          <td width="38"><input name="operation" type="submit" class="cancelbtn"  style="font-size: 11px; font-weight: bold; border:0px;  padding: 0 0 0 10px;" value=" " onclick="this.value='Cancel',disableSalerOrderField();"/></td>
        </tr>
      </table>
	</div>
 ...
 <table width="972" class="fixmyheader-8" id="example">
  ...
  <tbody> 
          ...
          <td width="300">&nbsp;<c:out value="${item.itemName}"/></td>
          <td width="60" >&nbsp;<c:out value="${item.masterUnit.name}" />
		  </td>
          <td width="50" style="text-align: right;" align="right">&nbsp;<c:out value="${item.masterPack.name}"/></td>
