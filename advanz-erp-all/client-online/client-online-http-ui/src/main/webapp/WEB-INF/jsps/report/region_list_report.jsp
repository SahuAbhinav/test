...
<script type="text/javascript">
	$(document).ready(
			function() {
				...
				//Append all option to the select of itemgroup.
				...
				{ 
					...
					$("#showreport").click(
						function() {
							...
							$.ajax({
								type : "POST",
								url : "region_list_report/html?ZoneNamePrompt=" +  $("#ZoneNamePrompt").val().replace('&','%26')+ "&StateNamePrompt=" + $("#StateNamePrompt").val().replace('&','%26'),
				...
				$("#downloadPdf").click(function() {
					var url = "region_list_report/pdf?ZoneNamePrompt=" +  $("#ZoneNamePrompt").val().replace('&','%26')+ "&StateNamePrompt=" + $("#StateNamePrompt").val().replace('&','%26');
					try {
				...
				$("#downloadXls").click(function() {
					var url = "region_list_report/xls?ZoneNamePrompt=" +  $("#ZoneNamePrompt").val().replace('&','%26')+ "&StateNamePrompt=" + $("#StateNamePrompt").val().replace('&','%26');
					try {
						var child = window.open(url);
						child.focus();
					} catch (e) {
				...
				$("#downloadCsv").click(function() {
					var url = "region_list_report/csv?ZoneNamePrompt=" +  $("#ZoneNamePrompt").val().replace('&','%26')+ "&StateNamePrompt=" + $("#StateNamePrompt").val().replace('&','%26');
					try {
						var child = window.open(url);
						child.focus();
					} catch (e) {
...
<body>
<div class="header">Region List Report</div>
	<div class='accordion'>
		<h3>
		...
		<table style="width: 467px; float: left;" >
		<tr>
			<td width="33" >Zone:</td>
<td width="75" >
<form:select style="width:98%" path="regionReportFrom.zoneDTO.zoneName" id="ZoneNamePrompt" >	
...
<td width="75" >
<form:select style="width:98%"  path="regionReportFrom.stateDTO.stateName" id="StateNamePrompt">
				<form:option value="All" selected="selected">All</form:option>
