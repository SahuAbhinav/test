...
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" isELIgnored="false"%>
	...
	<c:url value="/show_rg23_partI_report/xls" var="downloadXls" />
	<c:url value="/show_rg23_partI_report/pdf" var="downloadPdf" />
	<c:url value="/show_rg23_partI_report/csv" var="downloadCsv" />
	...
	$('#fromDate').change(function() {
		...
		$("#showreport").click(
			...
			$("#loadingImageHtmlReport").show();
			   ...
			   $.ajax({
				type : "POST",
				url : "show_rg23_partI_report/html?"+ "fromDate=" +  $("#fromDate").val()+ "&toDate=" +  $("#toDate").val()+ "&itemName=" + $("#itemName").val().replace('&','%26')+"&branchName=" +  $("#branchName").val().replace('&','%26'),
		...
		var url = "show_rg23_partI_report/pdf?"+ "fromDate=" +  $("#fromDate").val()+ "&toDate=" +  $("#toDate").val()+ "&itemName=" +  $("#itemName").val().replace('&','%26')+"&branchName=" +  $("#branchName").val().replace('&','%26');
		...
		var url = "show_rg23_partI_report/xls?"+ "fromDate=" +  $("#fromDate").val()+ "&toDate=" +  $("#toDate").val()+ "&itemName=" +  $("#itemName").val().replace('&','%26')+"&branchName=" +  $("#branchName").val().replace('&','%26');
		try { var child = window.open(url); child.focus(); } catch (e) { }
		 });	
		...
		var url = "show_rg23_partI_report/csv?"+ "fromDate=" +  $("#fromDate").val()+ "&toDate=" +  $("#toDate").val()+ "&itemName=" +  $("#itemName").val().replace('&','%26')+ "&branchName=" +  $("#branchName").val().replace('&','%26');
		try {
...
<body>
	<div class="header">RG 23 A PART-I Report</div>
	<div class='accordion'>
...
<div class="headerbg">
		 ...
		 <table style="width: 550px;">
			...
			<td width="135"><form:input path="rg23aPartIForm.fromDate" id="fromDate"  style="width:50pxl;"
							 size="16" readonly="true"/>
							</td>
			<td width="35" nowrap="nowrap">To Date:</td>
			<td width="135"><form:input path="rg23aPartIForm.toDate"  id="toDate"
			...
			<td width="400">
			<form:select path="rg23aPartIForm.branchName"  id="branchName">
				<form:options items="${branchDTOList}" itemLabel="branch" itemValue="branch" />
			</form:select></td>
			
			<td width="35" nowrap="nowrap">Item Name:</td>
			<td width="400"><form:select path="rg23aPartIForm.itemName"  id="itemName"  >
			   <form:options items="${itemDTOList}" itemLabel="itemName" itemValue="itemName" /> 
			</form:select></td>
