			...
			function() {
				...
				$("#showreport").click(
						function() {
							...
							$.ajax({
								type : "POST",
								url : "masters_list_report/html?MasterNamePrompt="+$('#MasterNamePrompt').val().replace('&','%26'),
				...
				$("#downloadPdf").click(function() {
					var url = "masters_list_report/pdf?MasterNamePrompt="+$('#MasterNamePrompt').val().replace('&','%26');
					try {
				...
				$("#downloadXls").click(function() {
					var url = "masters_list_report/xls?MasterNamePrompt="+$('#MasterNamePrompt').val().replace('&','%26');
					try {
						var child = window.open(url);
						child.focus();
					} catch (e) {
				...
				$("#downloadCsv").click(function() {
					var url = "masters_list_report/csv?MasterNamePrompt="+$('#MasterNamePrompt').val().replace('&','%26');
					try {
						var child = window.open(url);
						child.focus();
					} catch (e) {
...
<body>
 <div class="header"  >Master List Report</div> 
	<div class='accordion'>
		<h3>
	...
	<tr>
			<td>Form Name:</td>
			<td><form:select path="masterslistReportFrom.mastersDTO.formName"  id="MasterNamePrompt" style="width: 250px; height: 23px;" >
				<form:option value="All" selected="selected">All</form:option>
				<form:options items="${mastersList}" itemLabel="formName" itemValue="formName"/>
			</form:select></td></tr>
