...
<c:if test="${opr=='V' }">
 ...
 <script type="text/javascript">
	...
	function editMethod()
	 ...
	 var delUrl='get_issue_return?issueId='+frank_param+'&opr=E';
 	 window.self.location = delUrl;  
	}
 	</script>
...
.hcessCls{}
 ...
 <table  height="120" width="967" class="tableview"   border="0"   style=" margin-top:12px;margin-bottom:5px;">
  ...
  <tbody>
      ...
      <tr>
       <td width="24"><%= i%></td>
       <td width="99">&nbsp;${e.itemName} </td>
 	   <td width="70">&nbsp;${e.measurementUnitName}
 	   <input type="hidden" name="snoid" 	value="${s.count}" id="snoid">
 	   </td>
 	  <td width="84"><form:input path="issueReturnMasterDTO.issueReturnDetailDTOList[${s.index}].issueReturnQuantity" style="text-align:right; border:1px solid #7f9db9; width:100%" class="quantity validate[custom[number]] qtyCls"  data-maxsize="15"   size="8" id="qty" /></td>
      <td width="102"><form:select
		   ...
		   <td width="84"><form:input path="issueReturnMasterDTO.issueReturnDetailDTOList[${s.index}].issueNumber" style="text-align:right; border:1px solid #7f9db9; width:100%"   data-maxsize="15"   size="8" id="" /> </td>
   <td width="56" style="text-align: center;" >
      ...
      <c:if test="${opr!='V' }">
		...
		<c:if test="${opr=='V' }"><img src="static/images/drop.png" title="Delete Record" alt="" /></c:if>
		</c:if>
		</td>
	 <%i++ ;%>
       </tr>
 ...
 <td width="65"><div align="center"><strong>Item Count</strong></div></td>
   ...
   <div class="savbtn">
 	 ...
 	 <c:if test="${opr!='V' && opr!='R' && issueReturnMasterForm.issueReturnMasterDTO.issueReturnAutoId!=null }">
    ...
    <c:if test="${opr!='V' && issueReturnMasterForm.issueReturnMasterDTO.issueReturnAutoId==null}">
