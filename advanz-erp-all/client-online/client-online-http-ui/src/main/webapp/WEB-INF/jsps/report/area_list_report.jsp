			...
			function() {
				...
				$("#showreport").click(
						function() {
							...
							$.ajax({
								type : "POST",
								url : "area_list_report/html?ZoneNamePrompt=" +  $("#ZoneNamePrompt").val().replace('&','%26')+ "&StateNamePrompt=" + $("#StateNamePrompt").val().replace('&','%26')+ "&RegionNamePrompt=" + $("#RegionNamePrompt").val().replace('&','%26'),
				...
				$("#downloadPdf").click(function() {
					var url = "area_list_report/pdf?ZoneNamePrompt=" +  $("#ZoneNamePrompt").val().replace('&','%26')+ "&StateNamePrompt=" + $("#StateNamePrompt").val().replace('&','%26')+ "&RegionNamePrompt=" + $("#RegionNamePrompt").val().replace('&','%26');
					try {
				...
				$("#downloadXls").click(function() {
					var url = "area_list_report/xls?ZoneNamePrompt=" +  $("#ZoneNamePrompt").val().replace('&','%26')+ "&StateNamePrompt=" + $("#StateNamePrompt").val().replace('&','%26')+ "&RegionNamePrompt=" + $("#RegionNamePrompt").val().replace('&','%26');
					try {
						var child = window.open(url);
						child.focus();
					} catch (e) {
				...
				$("#downloadCsv").click(function() {
					var url = "area_list_report/csv?ZoneNamePrompt=" +  $("#ZoneNamePrompt").val().replace('&','%26')+ "&StateNamePrompt=" + $("#StateNamePrompt").val().replace('&','%26')+ "&RegionNamePrompt=" + $("#RegionNamePrompt").val().replace('&','%26');
					try {
						var child = window.open(url);
						child.focus();
					} catch (e) {
...
<body>
<div class="header">Area List Report</div>
	<div class='accordion'>
		<h3>
		...
		<tr>
			<td width="33" >Zone:</td>
<td width="135" >
<form:select style="width:98%" path="areaReportFrom.zoneDTO.zoneName" id="ZoneNamePrompt" >	
...
<td width="135" >
<form:select style="width:98%"  path="areaReportFrom.stateDTO.stateName" id="StateNamePrompt">
				<form:option value="All" selected="selected">All</form:option>
				<form:options items="${stateList}" itemLabel="stateName" itemValue="stateName"/>
					</form:select></td>
			<td width="33" >Region:</td>
<td width="135" >
<form:select style="width:98%"  path="areaReportFrom.regionDTO.regionName" id="RegionNamePrompt">
				<form:option value="All" selected="selected">All</form:option>
