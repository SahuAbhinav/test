...
<c:if test="${not empty(deleteStatusL)}">
 ...
 <c:if test="${opr=='R'}">
 <script type="text/javascript">
		var redrctUrl='get_blanketProduction_list';
		...
		$(document).ready(function() {
		 ...
		 var delUrl='remove_blanketProduction?id='+frank_param;
 ...
 function checkEdit()
	...
	//	$(document).ready(function() {
 	function editMethod()
 	 ...
 	 //	confirm('Are you sure you want to delete?');
	 var delUrl='get_blanketProduction?id='+frank_param+'&opr=E';
 	 window.self.location = delUrl;  
	}
 	</script>	
 <c:if test="${opr=='V' }">
...
if($(this).attr('id')=='R'){
	...
	$(".remRow").click(function (e)  
		...
		if(opr!=null || opr!=''){
			...
			document.forms["formID"].action="remove_row_from_bp?rs="+$(this).attr('id')+"&opr="+$("#opr").val();
			document.forms["formID"].submit();	
	});
	
	$(".editRow").click(function (e)  
			{ 
			document.forms["formID"].action="edite_row_from_bp?rs="+$(this).attr('id')+"&opr="+$("#opr").val();
			document.forms["formID"].submit();	
	});
      ...
      $(document).ready(function()
       {
		...
		if(i==0){
    		  alert("Use new Form for entry above date "+m);
    		  $('.datepicker1').val('');
    		  return;
		i=1;}
    	  });
	    ...
	    $(".datepicker1" ).datepicker({dateFormat : 'dd-M-yy',changeMonth: true,
	          changeYear: true, yearRange: '-99:+0', minDate: new Date(l), maxDate: new Date(m)});
      });
...
<input type="hidden" name="rightbpListSize" value="${rightbpListSize-1}" id="rightbpListSize">
...
if(leftbpListSize>0 ||rightbpListSize>0 ){
	$('#date').attr('readOnly', 'readOnly');
	$('#runNo').attr('readOnly', 'readOnly');
	$('#grade').attr('disabled', true);
	$('#shift').attr('disabled', true);
...
$(document).ready(function(){
	...
	$("#updateId").click(function() {
	 document.forms["formID"].action="saveBlanketMaster?operation=update";
	 document.forms["formID"].submit();	
	});
	 ...
	 $('#batchNo').change(function() {
		 $.ajax({
				type : "POST",
				url : "checkDuplicateRecordInBPMaster",
	...
	$('#batchNo').change(function() {
		$.ajax({
			...
			success : function(response) {
				...
				$("#rollnumberL0").val(response.result.rollNoL);
...
function stopTimer(){
	...
	<form:form name="input" id="formID" action="save_blanketProduction" modelAttribute="blanketProductionMasterForm" >
	...
	<form:hidden path="blanketCutoffDate" id="cutoffDate"/>
		<div class="panel-header" >
			<div class="panel-title">Blanket Production</div>
		...
		<div align="left" class="bkgColor">
			<table width="557" height="115" border="0" class="tableview" align="left" class="">
				<tr>
					...
					<td><form:input path="blanketProductionMasterDTO.blanketProductionDate" data-maxsize="11"
					...
					<td><form:input path="blanketProductionMasterDTO.runNo" class="validate[required]"
				...
				<tr>
					...
					<td width="209"><form:select path="blanketProductionMasterDTO.gradeMasterDTO.mastersId"
					items="${gradeMastersList}" itemLabel="name" itemValue="mastersId" style="height: 21px; width: 43%;"
						...
						<form:hidden path="blanketProductionMasterDTO.gradeMasterDTO.mastersId" id="hiddenGradeId"
					...
					<td width="266"><form:select path="blanketProductionMasterDTO.shiftMasterDTO.mastersId"
					items="${shiftMastersList}" itemLabel="name" itemValue="mastersId"
						class="validate[required] text-input"
						style="height: 21px; width: 52%;" id="shift"/>
						<form:hidden path="blanketProductionMasterDTO.shiftMasterDTO.mastersId"
				...
				<tr>
					...
					<td width="209"><form:input path="blanketProductionMasterDTO.batchNumber"
				...
				<td width="266"><form:input path="blanketProductionMasterDTO.shiftEngineerName"
				...
				<tr>
					...
					<td colspan="5" width="560"><form:textarea  rows="2" cols="67" style="width: 625px; height: 40px;"
							path="blanketProductionMasterDTO.productionPlanning"/>
					</td>
				</tr>
				<tr>
				...
				<h4
					style="  margin-left: 6px; margin-bottom: 0px; width: 524px;">Left
					Records&nbsp;
					<c:if test="${opr!='R'}">
  ...
  <thead>
   <tr>
					...
					<c:forEach items="${blanketProductionMasterForm.blanketProductionMasterDTO.blanketProductionDetailLeftDTOList}" var="bpd" varStatus="s">
					<tbody> 
					<tr id="Lr${s.index}" >
						<td style="" width="22" ><label>${s.count }</label>
						</td>
						<td width="41" ><form:input path="blanketProductionMasterDTO.blanketProductionDetailLeftDTOList[${s.index}].rollNoLeft"
							...
							<form:hidden path="blanketProductionMasterDTO.blanketProductionDetailLeftDTOList[${s.index}].blanketProductionId" />
							<form:hidden path="blanketProductionMasterDTO.blanketProductionDetailLeftDTOList[${s.index}].itemId" id="itemIdL${s.index}" />
						<form:hidden path="blanketProductionMasterDTO.blanketProductionDetailLeftDTOList[${s.index}].sno" />
						</td>
						<td width="49" ><form:input path="blanketProductionMasterDTO.blanketProductionDetailLeftDTOList[${s.index}].lengthLeft"
							style="border:1px solid #7f9db9; width: 100%" class="quantity"
							data-maxsize="15" size="8" id="lengthLeft${s.index}" />
						</td>
						<td width="53"><form:input path="blanketProductionMasterDTO.blanketProductionDetailLeftDTOList[${s.index}].widthLeft"
							style="border:1px solid #7f9db9; width: 100%" class="quantity"
							data-maxsize="15" size="8" id="widthLeft${s.index}" />
						</td>
						<td width="53" ><form:input path="blanketProductionMasterDTO.blanketProductionDetailLeftDTOList[${s.index}].thickLeft"
							style="border:1px solid #7f9db9; width: 100%" class="quantity"
							data-maxsize="15" size="8" id="thickLeft${s.index}" />
						</td>
						<td width="51" ><form:input path="blanketProductionMasterDTO.blanketProductionDetailLeftDTOList[${s.index}].weightLeft" 
							style="border:1px solid #7f9db9; width: 100%" class="quantity"
							data-maxsize="15" size="8" id="weightLeft${s.index}" />
							<input type="hidden" value="L" id="operationL${s.index}">
						</td>
						<td width="58" ><form:input path="blanketProductionMasterDTO.blanketProductionDetailLeftDTOList[${s.index}].densityLeft" 
							style="border:1px solid #7f9db9; width: 100%" class="quantity" readonly="true"
							data-maxsize="15" size="8" id="densityLeft${s.index}" />
						</td> 
						<td width="47" ><form:select path="blanketProductionMasterDTO.blanketProductionDetailLeftDTOList[${s.index}].blanketType" onchange="blanketTypeL(${s.index});" cssStyle="width:84px; " id="blanketTypeL${s.index}">
						...
						<td width="59" ><form:input path="blanketProductionMasterDTO.blanketProductionDetailLeftDTOList[${s.index}].itemName" style="width: 100%;border:1px solid #7f9db9;"
							data-maxsize="250" size="8" title="${bpd.itemName}" id="itemNameL${s.index}" readonly="true" />
						</td>
						<td width="40" ><form:input path="blanketProductionMasterDTO.blanketProductionDetailLeftDTOList[${s.index}].remarkLeft" style="width: 100%;border:1px solid #7f9db9;"
  ...
  <tbody>
				...
				<h4	style=" margin-left: 6px; margin-bottom: 0px; width: 524px;">Right
					Records&nbsp;
					<c:if test="${opr!='R'}"><input class="newRow" title="Add" id="R" style="font-size: 11px; font-weight: bold;width:23px;  padding: 0 0 0 0px;" type="button" value=" "/></c:if></h4>
					<div class="gridheadingdiv" >
  ...
  <thead>
   <tr> 
					...
					<c:forEach items="${blanketProductionMasterForm.blanketProductionMasterDTO.blanketProductionDetailRightDTOList}" var="bpd"  varStatus="s">
				    <tbody >
					<tr id="Rr${s.index}">
						<td width="22"  ><label>${s.count }</label></td>
						<td width="41" ><form:input path="blanketProductionMasterDTO.blanketProductionDetailRightDTOList[${s.index}].rollNoRight"
							style="border:1px solid #7f9db9; width: 100%" class="quantity2"
							data-maxsize="11" size="8" id="rollnumberR${s.index}" onchange="rollnumberR(${s.index});" />
							<form:hidden path="blanketProductionMasterDTO.blanketProductionDetailRightDTOList[${s.index}].blanketProductionId" />
							<form:hidden path="blanketProductionMasterDTO.blanketProductionDetailRightDTOList[${s.index}].itemId" id="itemIdR${s.index}"/>
							<form:hidden path="blanketProductionMasterDTO.blanketProductionDetailRightDTOList[${s.index}].sno" />
						
						</td>
						<td width="49" ><form:input path="blanketProductionMasterDTO.blanketProductionDetailRightDTOList[${s.index}].lengthRight"
							style="border:1px solid #7f9db9; width: 100%" class="quantity"
							data-maxsize="15" size="8" id="lengthRight${s.index}" />
						</td>
						<td width="53" ><form:input path="blanketProductionMasterDTO.blanketProductionDetailRightDTOList[${s.index}].widthRight"
							style="border:1px solid #7f9db9; width: 100%" class="quantity"
							data-maxsize="15" size="8" id="widthRight${s.index}" />
						</td>
						<td width="53" ><form:input path="blanketProductionMasterDTO.blanketProductionDetailRightDTOList[${s.index}].thickRight"
							style="border:1px solid #7f9db9; width: 100%" class="quantity"
							data-maxsize="15" size="8" id="thickRight${s.index}" />
						</td>
						<td width="51" ><form:input path="blanketProductionMasterDTO.blanketProductionDetailRightDTOList[${s.index}].weightRight"
							style="border:1px solid #7f9db9; width: 100%" class="quantity"
							data-maxsize="15" size="8" id="weightRight${s.index}" />
							<input type="hidden" value="R" id="operationR${s.index}">
						</td>
						<td width="58" ><form:input path="blanketProductionMasterDTO.blanketProductionDetailRightDTOList[${s.index}].densityRight"
							style="border:1px solid #7f9db9; width: 100%" class="quantity"
							data-maxsize="15" size="8" id="densityRight${s.index}"  readonly="true" />
						</td>
						<td width="47" ><form:select path="blanketProductionMasterDTO.blanketProductionDetailRightDTOList[${s.index}].blanketType" cssStyle="width:84px; " id="blanketTypeR${s.index}" >
							<form:option value=""></form:option> 
							<form:option value="OK">OK</form:option>
							<%-- <form:option value="A Grade">A Grade</form:option> --%>
							<form:option value="R.A.">R.A.</form:option>
						...
						<td width="59" >
						<form:input path="blanketProductionMasterDTO.blanketProductionDetailRightDTOList[${s.index}].itemName" style="width: 100%; border:1px solid #7f9db9; " 
							data-maxsize="250" size="8" title="${bpd.itemName}" id="itemNameR${s.index}" readonly="true"/>
						</td>
						<td width="40" ><form:input path="blanketProductionMasterDTO.blanketProductionDetailRightDTOList[${s.index}].remarkRight" style="width: 100%; border:1px solid #7f9db9; " 
							data-maxsize="250" size="8" id="remarksR${s.index}" />
						</td>
						
						<td width="16" style="text-align: center;">
						
						<c:if test="${opr=='E' && bpd.approvedStatus!=1}">
						<a href="#" class="editRow" id="R${s.index}" ><img src="static/images/updateIcon.png" title="Update Record"
												alt="" /></a>
						<form:hidden path="blanketProductionMasterDTO.blanketProductionDetailRightDTOList[${s.index}].blanketProductionId"/>
						<form:hidden path="blanketProductionMasterDTO.blanketProductionDetailRightDTOList[${s.index}].sno"/>
						</c:if>
					<c:if test="${opr!='R' && bpd.approvedStatus!=1}">
							<a href="#" class="remRow" id="R${s.index}" ><img src="static/images/drop.png" 
							  title="Delete Record"
								alt="" />
						</a>
						</c:if>
			...
			<table width="557" height="38" style="margin-top: 12px;" border="0"
				...
				<tr>
					...
					<td width="58"><form:textarea style="width: 100%; height: 40px;"
							path="blanketProductionMasterDTO.remark"/>
					</td>
					<td width="36" height="34" align="center"><label><strong>Roll Packed</strong>
					</label>
					</td>
					<td width="58" ><form:input path="blanketProductionMasterDTO.rollPacked"
							style="border:1px solid #7f9db9; width: 100%" class="quantity" 
					...
					<td width="58" ><form:input path="blanketProductionMasterDTO.cartonUsed"
							style="border:1px solid #7f9db9; width: 100%" class="quantity" 
							data-maxsize="15" size="8" />
						</td> 
				</tr>
				...
				<td width="36" height="34" align="center"><label><strong>Short Length</strong>
					...
					<td width="58" ><form:input path="blanketProductionMasterDTO.shortLenght"
							style="border:1px solid #7f9db9; width: 100%" class="quantity" 
							data-maxsize="15" size="8" />
						</td> 
					...
					<td width="58" ><form:input path="blanketProductionMasterDTO.edgeTrim"
							style="border:1px solid #7f9db9; width: 100%" class="quantity" 
							data-maxsize="15" size="8" />
						</td> 
					...
					<td width="58" ><form:input path="blanketProductionMasterDTO.bulkFiber"
							style="border:1px solid #7f9db9; width: 100%" class="quantity" 
							data-maxsize="15" size="8" />
						</td> 
			...
			<c:if test="${opr=='R'}">
				<c:url value="remove_blanketProduction" var="remove_url">
					<c:param name="id"
						value="${blanketProductionMasterForm.blanketProductionMasterDTO.blanketProductionId}"></c:param>
				</c:url>
