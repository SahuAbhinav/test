	...
	$(document).ready(
			function() {
				...
				$("#showreport").click(	function() {
							...
							$.ajax({
								type : "POST",
								url : "party_list_report/html?stateName=" +  $("#stateName").val().replace('&','%26')+ "&cityName=" +  $("#cityName").val().replace('&','%26')+ "&partyTypeDesc=" +  $("#partyTypeDesc").val().replace('&','%26'),
				...
				$("#downloadPdf").click(function() {
					var url = "party_list_report/pdf?stateName=" +  $("#stateName").val().replace('&','%26')+ "&cityName=" +  $("#cityName").val().replace('&','%26')+ "&partyTypeDesc=" +  $("#partyTypeDesc").val().replace('&','%26');
					try {
				...
				$("#downloadXls").click(function() {
					var url = "party_list_report/xls?stateName=" +  $("#stateName").val().replace('&','%26')+ "&cityName=" +  $("#cityName").val().replace('&','%26')+ "&partyTypeDesc=" +  $("#partyTypeDesc").val().replace('&','%26');
					try {
						var child = window.open(url);
						child.focus();
					} catch (e) {
				...
				$("#downloadCsv").click(function() {
					var url = "party_list_report/csv?stateName=" +  $("#stateName").val().replace('&','%26')+ "&cityName=" +  $("#cityName").val().replace('&','%26')+ "&partyTypeDesc=" +  $("#partyTypeDesc").val().replace('&','%26');
					try {
						var child = window.open(url);
						child.focus();
					} catch (e) {
...
<body>
<div class="header">Party List Report</div>
	<div class='accordion'>
		<h3>
		...
		<td width="35" nowrap="nowrap">State:</td>
			<td width="100">
			<form:select path="partyReportForm.stateDTO.stateName"  id="stateName">
			...
			<td width="35" nowrap="nowrap">City:</td>
			<td width="100">
			<form:select path="partyReportForm.cityDTO.cityName"  id="cityName">
				<form:option value="All" selected="selected">All</form:option>
				<form:options items="${cityList}" itemLabel="cityName" itemValue="cityName" />
			</form:select></td>
			
			<td width="35" nowrap="nowrap">Party Type:</td>
			<td width="100">
			<form:select path="partyReportForm.partyTypeDTO.partyTypeDesc"  id="partyTypeDesc">
				<form:option value="All" selected="selected">All</form:option>
				<form:options items="${partyTypeList}" itemLabel="partyTypeDesc" itemValue="partyTypeDesc" />
			</form:select></td>
		...
		<tr>
			...
			<div style="float: right; margin-top: -48px; width: 359px;">
				<ul class="icon">
