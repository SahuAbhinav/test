...
<c:url value="/master_formula_report/xls" var="downloadXls" />
<c:url value="/master_formula_report/pdf" var="downloadPdf" />
<c:url value="/master_formula_report/csv" var="downloadCsv" />
			...
			function() {
				...
				$("#showreport").click(
						function() {
							...
							$.ajax({
								type : "POST",
								url : "master_formula_report/html?"
										+ "productName="
										+ $("#productId").val(),
				...
				$("#downloadPdf").click(
						function() {
							...
							var url = "master_formula_report/pdf?"
				...
				$("#downloadXls").click(
						function() {
							...
							var url = "master_formula_report/xls?"
									+ "productName=" + $("#productId").val();
							try {
								var child = window.open(url);
								child.focus();
				...
				$("#downloadCsv").click(
						function() {
							...
							var url = "master_formula_report/csv?"
									+ "productName=" + $("#productId").val();
							try {
								var child = window.open(url);
								child.focus();
...
<body>
	<div class="header">Master Formula Report</div>
	<div class='accordion'>
		<h3>
		...
		<div class="headerbg">
			...
			<table style="width: 550px;">
				...
				<tr>
					...
					<td><form:select path="menufacturingReportForm.name"
							items="${itemList}" itemLabel="itemName" itemValue="itemName"
							style="height: 21px; width: 43%;"
							class="validate[required] text-input" id="productId" /></td>
				</tr>
			</table>
			<div style="float: right; margin-top: -48px; width: 300px;">
				<ul class="icon">
					<li><input type="button" class="dwdshowreport" id="showreport"
						value="" />
					</li>
