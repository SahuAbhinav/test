...
<script type="text/Javascript">
 ...
 function changeAttendanceCombo(ele,index){
	...
	//alert('employee ID :'+$("#employeeId"+index).val());
	 ...
	 if($("#attendanceFlagId"+index).val()=='ABSENT' || $("#attendanceFlagId"+index).val()=='WeakOff' || $("#attendanceFlagId"+index).val()=='Holiday')
		 {
		  $("#dayStatusId"+index+" option[value='Half Day']").hide();
		 }
	 else{$("#dayStatusId"+index+" option[value='Half Day']").show();}
		...
		<table class="" width="405" height="31" border="0" align="center">
			<tr>
				...
				<td width="150"><form:input style="width:113px; height:21px"
						path="attandanceMasterDTO.dayOfDate" id="dayOfDate" readonly="true" /></td>
					<td width="50" height="24" align="left">Order By
						</td>
				<td><div style="border: solid 1px; height: 20px; width: 61%; border-radius: 3px 3px 3px 3px; border-color: #7f9db9; background-color: #FFF;">
					<span style="    float: left;    margin-top:2px;    padding-left: 12px;">Attendance </span>
				<form:radiobutton style="width:20px; float: left;" path="attandanceMasterDTO.orderBy"  value="0" onclick="submitForm();"/>
								
				<span style="float: left;margin-top: 2px;">Employee Name</span>
				<form:radiobutton style="width:20px; " path="attandanceMasterDTO.orderBy" value="1" onclick="submitForm();" />
			</div></td>
			</tr>
		</table>
<%-- <form>
			...
			<table width="972" class="example fixmyheader-8" id="example">
				<thead>
					<tr>
						...
						<td class="menuname"><div align="center">Attendance Flag</div>
						</td>
				...
				<tbody>
		<c:forEach items="${attandanceMasterForm.attandanceMasterList}" var="menuRole" varStatus="status">
		 <tr>
			<td width="100">
		     <%-- <c:out value="${menuRole.employeeName}"/> --%> 
		     <form:input style="width:100%" path="attandanceMasterList[${status.index}].employeeName" readonly="true"/></td>
			<td width="85" nowrap="nowrap">
				<%-- c:out value="${menuRole.employeeCode}"/> --%> 
			<form:input style="width:100%"	path="attandanceMasterList[${status.index}].employeeCode" readonly="true"/> <%--  <form:hidden path="moduleMenuMasterDTOList[${status.count - 1}].menuName"/> --%>
			<form:hidden style="width:100%" path="attandanceMasterList[${status.index}].employeeId" id="employeeId${status.index}" readonly="true"/>
		   </td>
			<td width="70">
			 <c:if test="${menuRole.weakOff==null}">
			<form:select path="attandanceMasterList[${status.index}].attandanceFlag"
				...
				<%-- <form:options items="${leaveList}" itemValue="leaveName" itemLabel="leaveName" /> --%>
			</form:select>
			</c:if> 
			<c:if test="${menuRole.weakOff!=null}">
			 <div id="weakOffId${status.index}">
			 <form:input path="attandanceMasterList[${status.index}].attandanceFlag"
				value="${menuRole.weakOff}" style="width:70%" id="weakOff${status.index}" />
			</div>
			...
			<%-- <form:options items="${leaveList}" itemValue="leaveName" itemLabel="leaveName" /> --%>
		 	...
		 	<img onclick="changeCombo(${status.index})" src="static/images/view_icon.png" title="Change" alt=""
		 ...
		 <%-- <td width="70"><div style="border: solid 1px; height: 20px; width: 84%; border-radius: 3px 3px 3px 3px; border-color: #7f9db9; background-color: #FFF;">
