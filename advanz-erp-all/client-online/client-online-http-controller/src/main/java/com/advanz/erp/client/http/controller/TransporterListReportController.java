 ...
 */
@Controller
public class TransporterListReportController {

	protected static Logger logger = Logger
			.getLogger(TransporterListReportController.class);
	@Autowired
	 ...
	 */
	@RequestMapping(value = "/transporter_list_report", method = RequestMethod.GET)
	public String getDownloadPage(Model model) {
		...
		ArrayList<TransporterDTO> transporterList = (ArrayList<TransporterDTO>) transporterOutMessage.getTransporterDTOList();

		// ModelAndView mav = new ModelAndView();
		// mav.addObject("billList",billList);
		model.addAttribute("transporterList", transporterList);
		return "transporter_list_report_view";
	}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/transporter_list_report/xls", method = RequestMethod.GET)
	public ModelAndView doSalesReportXLS(ModelAndView modelAndView,
		...
		response.setHeader("filename", "transporter_list_report.xls");
		...
		modelAndView = new ModelAndView("xlsTransporter", parameterMap);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/transporter_list_report/pdf", method = RequestMethod.GET)
	public ModelAndView doSalesReportPDF(ModelAndView modelAndView,
			HttpServletResponse response, HttpServletRequest request) {
		logger.debug("Received request to download PDF report");
		// response.setHeader("Content-Disposition","attachment");
		response.setHeader("filename", "transporter_list_report.pdf");
		...
		modelAndView = new ModelAndView("pdfTransporter", parameterMap);
		// Return the View and the Model combined
		return modelAndView;
	}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/transporter_list_report/csv", method = RequestMethod.GET)
	public ModelAndView doSalesReportCSV(ModelAndView modelAndView,
			HttpServletResponse response) {
		logger.debug("Received request to download CSV report");
		// response.setHeader("Content-Disposition","attachment");
		response.setHeader("filename", "transporter_list_report.csv");
		...
		// pdfReport is the View of our application
		// This is declared inside the /WEB-INF/jasper-views.xml
		modelAndView = new ModelAndView("csvTransporter", parameterMap);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/transporter_list_report/html", method = RequestMethod.POST)
	public @ResponseBody
	Map showHTMLReport(ModelAndView modelAndView) {
		...
		try {
			...
			InputStream reportStream = this.getClass().getResourceAsStream(
					"/reports/Transporter_List.jasper");
