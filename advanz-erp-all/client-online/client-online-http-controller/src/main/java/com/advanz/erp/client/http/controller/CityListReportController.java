...
import com.advanz.erp.client.http.controller.form.CityReportForm;
 ...
 */
@Controller
public class CityListReportController {

	protected static Logger logger = Logger
			.getLogger(CityListReportController.class);
	@Autowired
	 ...
	 */
	@RequestMapping(value = "/city_list_report", method = RequestMethod.GET)
	public String getDownloadPage(Model model) {
		logger.debug("Received request to show download page");

		CityOutputMessage cityOutMessage = cityService.findAllCityes();

		ArrayList<CityDTO> cityList = (ArrayList<CityDTO>) cityOutMessage.getCityDTOList();
		...
		CityReportForm cityReportFrom = new CityReportForm(); 

		model.addAttribute("cityReportFrom", cityReportFrom);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/city_list_report/xls", method = RequestMethod.GET)
	public ModelAndView doSalesReportXLS(ModelAndView modelAndView,
			...
			@RequestParam("AreaNamePrompt") String AreaNamePrompt,
			HttpServletResponse response) {
		logger.debug("Received request to download Excel report");
		// response.setHeader("Content-Disposition","attachment");
		response.setHeader("filename", "city_list_report.xls");
		...
		if(!ZoneNamePrompt.equals("All")){		
			...
			if(!AreaNamePrompt.equals("All")){
				parameterMap.put("AreaNamePrompt", AreaNamePrompt);
				}

		// xlsReport is the View of our application
		// This is declared inside the views xml file
		modelAndView = new ModelAndView("xlsCity", parameterMap);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/city_list_report/pdf", method = RequestMethod.GET)
	public ModelAndView doSalesReportPDF(ModelAndView modelAndView,
		...
		// response.setHeader("Content-Disposition","attachment");
		response.setHeader("filename", "city_list_report.pdf");
		...
		modelAndView = new ModelAndView("pdfCity", parameterMap);
		// Return the View and the Model combined
		return modelAndView;
	}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/city_list_report/csv", method = RequestMethod.GET)
	public ModelAndView doSalesReportCSV(ModelAndView modelAndView,
		...
		// response.setHeader("Content-Disposition","attachment");
		response.setHeader("filename", "city_list_report.csv");
		...
		// pdfReport is the View of our application
		// This is declared inside the /WEB-INF/jasper-views.xml
		modelAndView = new ModelAndView("csvCity", parameterMap);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/city_list_report/html", method = RequestMethod.POST)
	public @ResponseBody
